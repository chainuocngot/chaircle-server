import { createZodDto } from 'nestjs-zod';
import {
  ChangePasswordBodySchema,
  ChangePasswordResSchema,
  GetGoogleAuthorizeUrlResSchema,
  GoogleOAuthCallbackQuerySchema,
  GoogleOAuthCallbackResSchema,
  GoogleOAuthStateSchema,
  LoginBodySchema,
  LoginResSchema,
  LogoutBodySchema,
  LogoutResSchema,
  RefreshTokenBodySchema,
  RefreshTokenResSchema,
  RegisterBodySchema,
  RegisterResSchema,
  ResetPasswordBodySchema,
  ResetPasswordQuerySchema,
  ResetPasswordResSchema,
  SendOtpBodySchema,
  SendOtpResSchema,
  VerifyForgotPasswordOtpBodySchema,
} from 'src/routes/auth/auth.model';

export class RegisterBodyDto extends createZodDto(RegisterBodySchema) {}

export class RegisterResDto extends createZodDto(RegisterResSchema) {}

export class LoginBodyDto extends createZodDto(LoginBodySchema) {}

export class LoginResDto extends createZodDto(LoginResSchema) {}

export class LogoutBodyDto extends createZodDto(LogoutBodySchema) {}

export class LogoutResDto extends createZodDto(LogoutResSchema) {}

export class RefreshTokenBodyDto extends createZodDto(RefreshTokenBodySchema) {}

export class RefreshTokenResDto extends createZodDto(RefreshTokenResSchema) {}

export class SendOtpBodyDto extends createZodDto(SendOtpBodySchema) {}

export class SendOtpResDto extends createZodDto(SendOtpResSchema) {}

export class VerifyForgotPasswordOtpBodyDto extends createZodDto(
  VerifyForgotPasswordOtpBodySchema,
) {}

export class ResetPasswordQueryDto extends createZodDto(ResetPasswordQuerySchema) {}

export class ResetPasswordBodyDto extends createZodDto(ResetPasswordBodySchema) {}

export class ResetPasswordResDto extends createZodDto(ResetPasswordResSchema) {}

export class ChangePasswordBodyDto extends createZodDto(ChangePasswordBodySchema) {}

export class ChangePasswordResDto extends createZodDto(ChangePasswordResSchema) {}

export class GetGoogleAuthorizeUrlResDto extends createZodDto(GetGoogleAuthorizeUrlResSchema) {}

export class GoogleOAuthCallbackQueryDto extends createZodDto(GoogleOAuthCallbackQuerySchema) {}

export class GoogleOAuthCallbackResDto extends createZodDto(GoogleOAuthCallbackResSchema) {}

export class GoogleOAuthStateDto extends createZodDto(GoogleOAuthStateSchema) {}
